...
import { fetchSubscriptionStatus } from "./subscription-cache";

const PRICING_URL = "https://www.contextmover.com/pricing";
...
export default function ProGate({ children }: ProGateProps) {
  ...
  useEffect(() => {
    void fetchSubscriptionStatus().then((res) => {
      setIsPro(Boolean(res?.isPro));
      setIsLoading(false);
    });
  }, []);
