...
//
// Auth model:
//   chrome.identity.launchWebAuthFlow — the only path used pre-publish.
//   Requires `https://<extension-id>.chromiumapp.org/` to be added as an
...
//   client in Google Cloud Console, then consider re-enabling
//   chrome.identity.getAuthToken as a fast-path.
//
...
class DriveClient {
  ...
  private async getToken(interactive = false): Promise<string | null> {
    ...
    // 2. Non-interactive: try silent refresh via launchWebAuthFlow with prompt=none.
    if (!interactive) {
    ...
    // 3. Interactive launchWebAuthFlow.
  ...
  /**
   * OAuth via chrome.identity.launchWebAuthFlow — the only auth path.
  ...
  /** Silent (non-interactive) token refresh using prompt=none. */
  private callLaunchWebAuthFlowSilent(): Promise<string | null> {
    const manifest = chrome.runtime.getManifest() as chrome.runtime.Manifest & {
      oauth2?: { client_id?: string; scopes?: string[] };
    };
    const clientId = manifest.oauth2?.client_id;
    const scopes = manifest.oauth2?.scopes ?? [];
    if (!clientId || !chrome.identity?.launchWebAuthFlow) return Promise.resolve(null);
  ...
  /** Proactively refresh token if close to expiry. Called at the start of isConnected(). */
  private async maybeRefreshToken(): Promise<void> {
    try {
      const stored = await chrome.storage.local.get([FLOW_TOKEN_KEY, FLOW_TOKEN_AT_KEY]);
      const at = stored[FLOW_TOKEN_AT_KEY] as number | undefined;
      if (!at) return;
      // If token expires in < 5 min, try silent refresh.
      const remaining = FLOW_TOKEN_TTL_MS - (Date.now() - at);
      if (remaining < 5 * 60 * 1000) {
        const refreshed = await this.callLaunchWebAuthFlowSilent();
        if (refreshed) await this.writeFlowToken(refreshed);
      }
    } catch {
  ...
  /** Clear cached tokens + clear local drive state. */
  async disconnect(): Promise<void> {
    // Clear our launchWebAuthFlow token cache.
