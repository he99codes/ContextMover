...
export class AttentionEngine {
  ...
  async scoreMessagesForSummarization(
    ...
    // 60s: generous enough for large sessions (500+ chunks) and single-threaded WASM.
    // Callers that need shorter deadlines (e.g. T2 hot path) race against their own cutoff.
...
export async function detectHardware(): Promise<HardwareProfile> {
  ...
  if (_lastLoggedProfile !== tier) {
    _lastLoggedProfile = tier;
