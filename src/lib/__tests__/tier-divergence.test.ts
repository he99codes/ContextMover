...
// path from src/lib/__tests__/ → ContextMover root (3 levels up)
...
describe('3A-1: tier files are distinct and size-ordered', () => {
  ...
  it('T3 <= 35% of T1 (top-50/200 messages = ~25% of content + overhead)', () => {
    expect(t3.charCount, 'T3 should be at most 35% of T1').toBeLessThanOrEqual(
      t1.charCount * 0.35
    )
  })
