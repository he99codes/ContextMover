...
vi.mock("../../db", () => ({
  dexieDb: {
    sessionHashes: { get: vi.fn(async () => undefined), delete: vi.fn(async () => {}), put: vi.fn(async () => {}) },
    chunkEmbeddings: { where: () => ({ equals: () => ({ count: async () => 0, toArray: async () => [], delete: async () => {} }) }) },
    pendingIndex: { put: vi.fn(async () => {}), delete: vi.fn(async () => {}) },
  },
}));
...
describe("SemanticIndex queue — timeout guard (CRITICAL fix)", () => {
  it("rejects a hung offscreen job instead of stalling the queue forever", async () => {
    vi.useFakeTimers();
    ...
    // Advance past the 90s offscreen-index timeout.
