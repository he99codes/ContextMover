...
export function chunkMessages(messages: Message[]): Chunk[] {
  const chunks: Chunk[] = [];
  // For large sessions (>100 messages), use larger chunks to reduce total count.
  const targetTokens = messages.length > 100 ? 1500 : TARGET_CHUNK_TOKENS;
  ...
  for (let msgIdx = 0; msgIdx < messages.length; msgIdx++) {
    ...
    for (const part of parts) {
      ...
      const proseChunks = splitProse(cleaned, targetTokens);
