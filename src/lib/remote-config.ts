  ...
  try {
    ...
    if (cached && isValidConfig(cached)) {
      if (age > 7 * 24 * 60 * 60 * 1000) {
        console.warn(`[CM:config] Cached remote config is ${Math.round(age / (24 * 60 * 60 * 1000))} days old — selectors may be stale`);
      }
...
const _selectorCache = new Map<string, { value: PlatformSelectors | null; ts: number }>();
...
export async function getPlatformSelectors(platform: string): Promise<PlatformSelectors | null> {
  const cached = _selectorCache.get(platform);
  if (cached && Date.now() - cached.ts < SELECTOR_CACHE_TTL) return cached.value;
  const config = await getRemoteConfig();
  const platformConfig = config?.find(p => p.platform_id === platform);
  const result = (!platformConfig || !platformConfig.is_enabled) ? null : platformConfig.selectors;
  _selectorCache.set(platform, { value: result, ts: Date.now() });
  return result;
}
