...
import { semanticIndex, pauseBackgroundIndexing, resumeBackgroundIndexing } from "@/lib/semantic-index/index";
...
import { pickBestKnown, shouldRejectIncoming } from "@/lib/capture/capture-merge";
    ...
    try {
      ...
      try {
        await new Promise<void>((res, rej) => {
          ...
          const t = setTimeout(() => { chrome.runtime.onMessage.removeListener(l); rej(new Error('offscreen_ready_timeout')); }, 20_000);
...
// Cached offscreen liveness check — avoids calling hasDocument() on every message.
...
const DRIVE_PULL_FROM_LIST_COOLDOWN_MS = 15_000;
...
chrome.runtime.onStartup.addListener(async () => {
  ...
  if (driveValid) {
    ...
    chrome.alarms.create("drive-sync-periodic", { periodInMinutes: 2 });
  }
  ...
  setTimeout(() => {
    void (async () => {
      try {
        ...
        // Collect un-indexed sessions first, then sort smallest-first so short
        // sessions complete quickly and don't block behind a 200+ chunk session.
        const unindexed: typeof recentSessions = [];
        for (const session of recentSessions) {
          if (session.messages.length > 0) {
            const chunkCount = await dexieDb.chunkEmbeddings.where('sessionId').equals(session.id).count();
            if (chunkCount === 0) unindexed.push(session);
          }
        }
        // Sort by total content length (not message count) so sessions that will
        // produce many chunks (e.g. a 2-message session with huge AI responses)
        // are queued last and don't block shorter sessions for 120+ seconds.
        unindexed.sort((a, b) => {
          const aChars = a.messages.reduce((s, m) => s + m.content.length, 0);
          const bChars = b.messages.reduce((s, m) => s + m.content.length, 0);
          return aChars - bChars;
        });
        for (const session of unindexed) {
          const sessionChars = session.messages.reduce((s, m) => s + m.content.length, 0);
          console.log(`[CM:startup] Found un-indexed session ${session.id} (${session.messages.length} msgs, ${sessionChars} chars), re-queuing`);
    ...
    void (async () => {
      try {
        ...
        for (const job of pending) {
          // Chunk 35: skip entries older than 24h — session may no longer exist.
          if (job.createdAt && Date.now() - job.createdAt > 24 * 60 * 60 * 1000) {
            console.debug(`[CM:startup] skipping stale pendingIndex entry ${job.sessionId} (>24h old)`);
...
chrome.runtime.onInstalled.addListener(() => {
  chrome.alarms.create(REMOTE_CONFIG_ALARM, { periodInMinutes: REMOTE_CONFIG_PERIOD_MIN });
  chrome.alarms.create("drive-sync-periodic", { periodInMinutes: 2 });
  chrome.alarms.create("offscreen-keepalive", { periodInMinutes: 0.25 }); // 15s
});

// Chunk 24: Keepalive port — while sidebar is open, SW stays alive.
chrome.runtime.onConnect.addListener((port) => {
  if (port.name === "sidebar-keepalive") {
    // Port stays open until sidebar disconnects; SW won't be evicted.
...
supabase.auth.onAuthStateChange(async (event, session) => {
  if (session?.access_token) {
    ...
    console.debug("[CM:auth] token refreshed, event:", event);
  }
  if (event === "SIGNED_OUT") {
...
chrome.alarms.onAlarm.addListener((alarm) => {
  ...
  if (alarm.name === "drive-sync-periodic") {
    // SYNC-2, SYNC-3: bidirectional sync every 2 min while Drive connected.
    void driveSyncManager.syncBidirectional();
  }
  if (alarm.name === "offscreen-keepalive") {
    // Wake the SW and check offscreen doc is still alive.
    chrome.storage.local.get('_ka', () => {});
    void isOffscreenAlive().then((alive) => {
      if (!alive) {
        void ensureOffscreenDocumentLocal();
      }
    });
  }
});
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  (async () => {
    ...
    switch (msg.type) {
      ...
      case "DRIVE_CONNECT": {
        ...
        try {
          ...
          if (connected) {
            ...
            // SYNC-2: ensures cross-extension changes propagate within 2 min.
            chrome.alarms.create("drive-sync-periodic", { periodInMinutes: 2 });
          }
      ...
      case "DRIVE_SYNC_NOW": {
        if (!isFromExtensionUI(sender)) { sendResponse({ error: "Unauthorized" }); return; }
        await driveSyncManager.initialSync();
        sendResponse({ ok: true });
        break;
      }
      case "CAPTURE_SESSION": {
        ...
        // Validate individual messages: must have role + content strings.
        const msgs = p.messages as unknown[];
        if (msgs.length > 0 && !msgs.every((m) =>
          typeof m === 'object' && m !== null &&
          typeof (m as Record<string, unknown>).role === 'string' &&
          typeof (m as Record<string, unknown>).content === 'string'
        )) {
          console.warn('[CM:sw] CAPTURE_SESSION invalid message structure');
          sendResponse({ error: 'CAPTURE_SESSION: invalid message structure' });
          break;
        }
      ...
      case "MIGRATE_CONTEXT": {
        ...
        // Only for Tier 3 — ONNX embedding is only needed for semantic retrieval.
        // Firing this for T1/T2 migrations queues an unnecessary ONNX job that
        // occupies the offscreen doc and adds queue latency to the actual migration.
        if (_tier === 3 && msg.payload?.sessionId) {
          void (async () => {
            try {
              await ensureOffscreenDocumentLocal();
      ...
      case "GET_SUBSCRIPTION_STATUS": {
        ...
        try {
          ...
          if (res.status === 403) {
            ...
            // Device limit exceeded — show explicit error, not silent free downgrade
            sendResponse({ plan: "free", error: true, deviceLimitExceeded: true, message: body.error ?? "Device limit reached" });
            break;
          }
          if (!res.ok) {
...
async function handleCaptureSession(payload: {
  ...
  writeTimers.set(session.id, setTimeout(async () => {
    ...
    console.debug('[CM:sw] SAVED to IndexedDB', { sessionId: toWrite.id, platform: toWrite.platform, messages: toWrite.messages.length, title: toWrite.title });
    void broadcastToViews({ type: "SESSIONS_UPDATED" });
    // Kick off background semantic indexing — fire & forget, never blocks capture
    if (semanticIndex.getQueueLength() >= 80) {
      console.warn(`[CM:sw] backpressure: deferring index for ${toWrite.id} by 15s`);
      setTimeout(() => backgroundIndex(toWrite).catch(() => {}), 15000);
    } else {
    ...
    // MetaPrompt pre-build: skip tiny sessions — building 12 platform prompts for
    // a 2-msg session costs more than on-demand computation at migration time.
    if (toWrite.messages.length >= 3) {
...
const _indexDirty = new Map<string, number>(); // sessionId → timestamp of last dirty signal
// Sessions whose background index was cancelled because a migration started.
// They are re-indexed once the migration completes.
const _indexCancelledByMigration = new Set<string>();
// Track re-index attempts per session to cap at 3 (Chunk 8).
const _reindexCounters = new Map<string, number>();
...
async function backgroundIndex(session: ContextSession): Promise<void> {
  ...
  if (activeMigrationInProgress) {
    _indexCancelledByMigration.add(session.id);
    console.debug(`[CM:sw:bgIdx] deferred ${session.id} — migration in progress`);
    return;
  }
  // Dedup: if this session is already being indexed, don't queue a duplicate.
  // Mark it dirty instead — once the in-flight job finishes we re-run once with
  // the latest session so messages added mid-index still get embedded.
  if (_indexingInFlight.has(session.id)) {
    _indexDirty.set(session.id, Date.now());
    console.debug(`[CM:sw:bgIdx] in-flight for ${session.id} — marked dirty for re-index`);
    return;
  }
  ...
  try {
    ...
    // Chunk 7: check offscreen is alive before indexing, recreate if missing.
    const alive = await ensureOffscreenAliveOrRecreate();
    if (!alive) {
      console.warn(`[CM:sw:bgIdx] offscreen unavailable for ${session.id} — skipping`);
      return;
    }
    await semanticIndex.indexSession(session);
    const dt = (performance.now() - t0).toFixed(0);
    console.debug(`[CM:sw:bgIdx] indexed session ${session.id} in ${dt}ms`);
    ...
    // Chunk 7: if offscreen crashed mid-index, try to recreate and retry once.
    if (msg.includes("offscreen") || msg.includes("Could not establish connection") || msg.includes("message port closed")) {
      _offscreenAliveCache = false;
      _offscreenAliveCacheAt = 0;
      console.warn(`[CM:sw:bgIdx] offscreen error for ${session.id}, attempting recreation + retry`);
      const recreated = await ensureOffscreenAliveOrRecreate();
      if (recreated) {
        try {
          await semanticIndex.indexSession(session);
          console.debug(`[CM:sw:bgIdx] retry succeeded for ${session.id}`);
          return;
        } catch (retryErr) {
          console.warn(`[CM:sw:bgIdx] retry also failed for ${session.id}:`, retryErr);
        }
      }
    }
    ...
    // If new messages arrived mid-index, re-run once with the freshest copy.
    // Chunk 8: max 3 re-indexes per session to prevent infinite loops.
    if (_indexDirty.has(session.id)) {
      const dirtyTime = _indexDirty.get(session.id)!;
      _indexDirty.delete(session.id);
      // Only re-index if the dirty signal arrived during this index job.
      const reindexCount = _reindexCounters.get(session.id) ?? 0;
      if (reindexCount < 3) {
        _reindexCounters.set(session.id, reindexCount + 1);
        const fresh = await db.getSession(session.id).catch(() => null);
        if (fresh) {
          console.debug(`[CM:sw:bgIdx] re-indexing ${session.id} (dirty, attempt ${reindexCount + 1}/3)`);
          void backgroundIndex(fresh).catch(() => {});
        }
      } else {
        console.warn(`[CM:sw:bgIdx] max re-index reached for ${session.id} — skipping`);
        _reindexCounters.delete(session.id);
      }
    } else {
      _reindexCounters.delete(session.id);
    }
...
/**
 * Re-index all sessions that were cancelled because a migration was in progress.
 * Called after every migration completion or error path.
 */
async function drainCancelledByMigration(): Promise<void> {
  if (_indexCancelledByMigration.size === 0) return;
  const ids = [..._indexCancelledByMigration];
  _indexCancelledByMigration.clear();
  for (const id of ids) {
    const fresh = await db.getSession(id).catch(() => null);
    if (!fresh) continue;
    // Skip re-indexing sessions that were priority-indexed during T3 migration.
    // Checking chunkEmbeddings prevents a spurious background job immediately
    // after migration that would show an "Indexing session" indicator to the user.
    const existingChunks = await dexieDb.chunkEmbeddings.where('sessionId').equals(id).count().catch(() => 0);
    if (existingChunks > 0) {
      console.debug(`[CM:sw:bgIdx] skipping post-migration re-index for ${id} (${existingChunks} chunks already present)`);
      continue;
    }
    console.debug(`[CM:sw:bgIdx] resuming deferred index for ${id} (migration completed)`);
    void backgroundIndex(fresh).catch(() => {});
  }
}
...
async function handleMigrateContext(
  ...
  pauseBackgroundIndexing(); // hold all queued background jobs until migration completes
  ...
  try {
    if (tier === 1) {
      ...
      if (stored) {
        ...
        try {
          // [CM-T2-ENHANCE] transformer-scored extraction — non-blocking: 1.5s deadline.
          // If the offscreen doc is busy (background indexing), fall through immediately
          // to heuristic and fire async scoring to warm the cache for the next migration.
          const T2_SCORE_INLINE_MS = 1_500;
          const scorePromise = attentionEngine.scoreMessagesForSummarization(session);
          const inlineCutoff = new Promise<ScoredMessage[]>((r) =>
            setTimeout(() => r([]), T2_SCORE_INLINE_MS)
          );
          scoredMessages = await Promise.race([scorePromise, inlineCutoff]);
          if (scoredMessages.length > 0) {
            console.log(`[CM:tier2] scored ${scoredMessages.length} messages for extraction (inline)`)
          } else {
            console.log('[CM:tier2] scoring deferred (offscreen busy) — using heuristic this run')
            // Async cache-warm: finish scoring in background, update stored summary for next run.
            void scorePromise.then(async (scores) => {
              if (scores.length === 0) return;
              try {
                const warmSummary = await buildTier2WithScoring(session, scores, payload.task);
                await semanticIndex.saveSummary(
                  session.id, 2, payload.task ?? null,
                  JSON.stringify(warmSummary), warmSummary.compressionRatio,
                  session.messages.length
                );
                console.log('[CM:tier2] async cache-warm complete — next migration will use scored result')
              } catch { /* best-effort */ }
            }).catch(() => {});
          }
        } catch (err) {
      ...
      try {
        ...
        // Keyword fallback means embeddings weren't ready yet — chunks are still
        // valid content ranked by keyword relevance. Accept them and proceed;
        // the warmup triggered above means next migration will use full semantic search.
        if (usedKeywordFallback) {
          console.warn('[CM:sw] Tier 3 used keyword fallback — proceeding with keyword-ranked chunks');
          reportProgress(78, 'Using keyword ranking (semantic warming up)...');
        }
        if (chunks.length === 0) {
