...
export function subscriptionRenewedEmail(email: string, gateway: "razorpay" | "stripe"): { subject: string; html: string } {
  const currency = gateway === "razorpay" ? "₹299/month" : "$5/month";
  return {
    subject: "ContextMover Pro renewed ✓",
    html: layout(`
      ${badge("SUBSCRIPTION RENEWED")}
      <div style="margin-top:16px"></div>
      ${h1("Your Pro subscription has been renewed")}
      ${p(`Hi ${email},`)}
      ${p("Your Pro subscription has been successfully renewed. Your unlimited migrations continue uninterrupted.")}
      ${divider()}
      <table width="100%" cellpadding="0" cellspacing="0">
        ${row("Plan", "Pro")}
        ${row("Billing", currency)}
      ...
      ${divider()}
      ${btn("Open Dashboard", "https://contextmover.com/dashboard")}
      ${p(`<span style="font-size:13px;color:#6B6B6B">To manage or cancel: Dashboard → Settings → Billing. Questions? <a href='mailto:support@contextmover.com' style='color:#00D26A'>support@contextmover.com</a></span>`)}
    `),
  };
}


export function proCancelledEmail(email: string, periodEnd?: string): { subject: string; html: string } {
  const stillActive = periodEnd && new Date(periodEnd).getTime() > Date.now();
  const expiryLine = stillActive
    ? `<p style="font-size:14px;color:#EAB308;margin:12px 0;"><strong>Your Pro features remain active until ${new Date(periodEnd).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}.</strong> After that, your account will move to the free tier.</p>`
    : "";
  const autopayWarning = `<div style="background:#1A1A1A;border:1px solid #EAB308/30;border-radius:8px;padding:16px;margin:16px 0;"><p style="color:#EAB308;font-size:14px;font-weight:600;margin:0 0 8px 0;">⚠️ Important: Cancel Auto-Pay</p><p style="color:#F5F5F5;font-size:13px;margin:0;">If you set up auto-pay (mandate) in Razorpay, please also cancel the auto-pay mandate in your <a href="https://dashboard.razorpay.com/app/mandates" style="color:#00D26A">Razorpay dashboard</a> to prevent future automatic charges.</p></div>`;
  return {
    ...
    html: layout(`
      ...
      ${h1("Pro cancelled")}
      ${p(`Hi ${email},`)}
      ${p("Your Pro subscription has been cancelled.")}
      ${expiryLine}
      ${autopayWarning}
      ${divider()}
      <table width="100%" cellpadding="0" cellspacing="0">
        ${row("Plan after expiry", "Free")}
...
export function refundRequestReceivedEmail(email: string): { subject: string; html: string } {
  return {
    subject: "Refund request received — ContextMover",
    html: layout(`
      ${badge("REFUND REQUEST", "#00D26A")}
      <div style="margin-top:16px"></div>
      ${h1("We've received your request")}
      ${p(`Hi ${email},`)}
      ${p("Your refund request has been submitted successfully. Our team will review it within 2 business days.")}
      ${p("You don't need to take any further action — we'll email you once your request has been processed.")}
      ${divider()}
      ${p(`<span style="font-size:13px;color:#6B6B6B">Questions? <a href='mailto:support@contextmover.com' style='color:#00D26A'>support@contextmover.com</a></span>`)}
    `),
  };
}
