...
export async function isDuplicateEvent(
  ...
  const { data } = await supabase
    .from("payment_events")
    .select("id")
    .eq("razorpay_event_id", gatewayEventId)
    .limit(1)
    .maybeSingle();
  return Boolean(data);
}
...
export async function logPaymentEvent(
  ...
  const { error } = await supabase.from("payment_events").insert({
    razorpay_event_id:        gatewayEventId,
...
export async function getUserUsage(
  ...
  return {
    month,
    simpleMigrations:    data?.tier1_count ?? 0,
    smartMigrations:     data?.tier2_count ?? 0,
    attentionMigrations: data?.tier3_count ?? 0,
    sessionsCount:       0,
  };
}
