...
import { proActivatedEmail } from "@/lib/emails/templates";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // [SECURITY] Rate limit: 10 verification attempts per minute per user.
    ...
    // Send congrats email (webhook may also send one — idempotency via payment_events)
    try {
      const { data: userData } = await admin.auth.admin.getUserById(user.id);
      const email = userData?.user?.email;
      if (email) {
        await sendEmail({ ...proActivatedEmail(email, "razorpay"), to: email, from: SENDERS.support });
      }
    } catch (emailErr) {
      console.warn("[verify-subscription] congrats email failed:", emailErr);
    }
