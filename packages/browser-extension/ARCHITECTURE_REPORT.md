...
> Code-free LLM reference. Updated June 2026.
...
└── lib/
    ...
    ├── db.ts               Dexie v8 schema: sessions, prompt_templates, prompt_assignments,
    │                       chunkEmbeddings, sessionHashes, storedSummaries,
    │                       retrievalCache, migrationQuality, metaPrompts,
    │                       pendingIndex, performanceMetrics
    ...
    │
    ├── perf-track.ts       P50/P90/P95/P99 latency telemetry: perfStart(), recordPerf(),
    │                       getPerfStats() — writes to performanceMetrics (Dexie v8)
    │
    ├── mcp/
...
## 9. System Status (June 2026)

> Previously this section described critical failures from an earlier assessment. All four issues have since been resolved. This section now reflects verified current status.

### 9.1 Attention Engine — ✅ FIXED

**Previously broken:** ONNX query embedding timed out on large sessions; `web-tree-sitter` failed to load.

**Current status:**
- Tier 3 migration timeout is now scaled dynamically: `60s + 1s × messageCount`, capped at 300s. A 200-message session gets a 260s budget vs the previous flat 120s.
- Background indexing timeout: `30s + 1s × messageCount`, capped at 240s.
- `web-tree-sitter` WASM is copied to `dist/` at build time; Vite config confirmed correct.
- Partial embedding integrity guard in `needsIndexing()`: if `chunkEmbeddings.count < stored.chunkCount × 0.8`, the session is re-queued for full re-index.

### 9.2 Capture Trigger — ✅ FIXED

**Previously broken:** Existing sessions and SPA navigation did not reliably trigger scraping.

**Current status:**
- MutationObserver in `shared.ts` fires on every DOM change; `URL_CHANGED` message triggers re-resolve of `sessionId` and a fresh scrape.
- `session-id.ts` strips platform-specific tracking params (Grok `?rid=`, ChatGPT non-conversation params, Perplexity all params) so revisiting a URL returns the same `sessionId`.
- Fetch-interceptor path (`fetch-interceptor.ts`) captures full API responses for ChatGPT (`/ces/v1/t`, `/backend-api/conversation/`) and proxies through `interceptor-bridge.ts`.
- `captureInFlight` dedup + shrink guard prevents DOM scrape from overwriting a larger network capture.

### 9.3 Scraper Brittleness & Admin API — ✅ FIXED

**Previously broken:** All scrapers broken; admin API returning 403.

**Current status:**
- All 6 platform scrapers updated to 2026 DOM structures with multi-strategy fallback chains (custom elements → data-testid → class patterns → structural heuristic).
- Gemini uses `user-query` / `model-response` custom elements as primary, with 5-level fallback.
- Grok uses `[data-testid="user-message"]` / `[data-testid="assistant-message"]` with `message-bubble` class fallback.
- Admin API `Authorization: Bearer` header now sent correctly; scraper config updates confirmed working.
- Remote config (`contextmover.com/config/selectors.json`, 1hr TTL) provides live CSS selector overrides without extension update.

### 9.4 Background Indexing Queue — ✅ FIXED

**Previously broken:** Queue overflow dropped indexing jobs silently.

**Current status:**
- `pendingIndex` Dexie table (v7) provides a persistent queue that survives SW restarts and Chrome close/open.
- Backpressure: if queue length ≥ 40, new jobs are deferred 60s instead of dropped.
- `_indexingInFlight` + `_indexDirty` flags prevent duplicate concurrent jobs while ensuring dirty sessions get re-indexed once the in-flight job finishes.
- Hardware gating: `minimal`-tier devices skip embedding entirely; keyword search fallback is transparent.

### 9.5 Remaining Smaller-Scope Issues

| Issue | Severity | Status |
|-------|----------|--------|
| Cross-profile incomplete embeddings (Section 13) | Medium | Known, mitigated by re-index on `chunkCount=0` | 
| Gemini injection: text may not always land in input on Angular re-renders | Low | Detailed logging added; root cause varies per Gemini version |
| Tier 3 on 200+ message sessions on `minimal` hardware still slow | Low | By design — user can switch to Tier 2 |
...
## 14. Production Observability & SLO System (June 2026)

### Architecture

A production-grade latency telemetry system was implemented in June 2026. It instruments all critical user-facing operations with zero performance overhead and stores P50/P90/P95/P99 data in IndexedDB for analysis.

**Components:**

| Component | File | Role |
|-----------|------|------|
| `PerformanceMetric` schema | `lib/db.ts` (v8) | Dexie table `++id, operation, timestamp` |
| `perfStart()` / `recordPerf()` | `lib/perf-track.ts` | Fire-and-forget timing recorder |
| `getPerfStats()` | `lib/perf-track.ts` | P50/P90/P95/P99 + SLO compliance per operation |
| `GET_PERF_STATS` SW handler | `service-worker.ts` | Exposes stats to sidebar (rolling window) |
| `EXPORT_PERF_METRICS` SW handler | `service-worker.ts` | Returns raw rows for offline analysis |
| `PerfStatsPanel` | `sidebar/PerfStatsPanel.tsx` | Visual dashboard in Settings panel |

**Query embedding cache** (`lib/semantic-index/index.ts`): 5-minute TTL, 50-entry cap. If the same query string is seen within 5 minutes, the offscreen ONNX embed call is skipped entirely. Expected 20-30% search P90 improvement on repeated queries.

### Instrumented Operations

| Operation | Where | What is measured |
|-----------|-------|-----------------|
| `capture_session` | `handleCaptureSession` | Entry → IDB write confirmed |
| `migrate_tier1` | `handleMigrateContext` | Entry → `sendResponse` success |
| `migrate_tier2` | `handleMigrateContext` | Entry → `sendResponse` success |
| `migrate_tier3` | `handleMigrateContext` | Entry → `sendResponse` success |
| `background_index` | `backgroundIndex` | `indexSession()` call duration |
| `semantic_search` | `SemanticIndex.retrieve()` | Both cosine and keyword-fallback paths |
| `drive_sync` | `pullFromDrive()` | Full pull cycle duration |

### SLO Targets (P90 budget)

| Operation | P90 Target | Rationale |
|-----------|-----------|-----------|
| `capture_session` | 2,000ms | Users expect captures to feel instant |
| `migrate_tier1` | 3,000ms | Full context file build is local |
| `migrate_tier2` | 6,000ms | Local summarizer, no network required |
| `migrate_tier3` | 15,000ms | ONNX inference + retrieval; user-triggered, expected wait |
| `background_index` | 30,000ms | Background, not user-blocking |
| `semantic_search` | 2,000ms | User-facing search box — must feel responsive |
| `drive_sync` | 10,000ms | Network-bound; background operation |

### Data Retention & Storage Safety

- **7-day rolling window**: rows older than 7 days are auto-deleted on every write.
- **500 rows per operation cap**: oldest rows evicted when limit reached.
- **Max IndexedDB footprint**: ~3.5 MB at cap (7 ops × 500 rows × ~1 KB/row).

### Accessing the Data

**In sidebar:** Settings panel → "Latency (7d)" table shows P50/P90/P99 + SLO pass/fail per operation.

**In browser console (raw analysis):**
```javascript
// Summary stats
const stats = await chrome.runtime.sendMessage({ type: 'GET_PERF_STATS' });
console.table(stats.stats);

// Raw rows for custom analysis
const { rows } = await chrome.runtime.sendMessage({ type: 'EXPORT_PERF_METRICS' });
const byOp = {};
rows.forEach(r => { (byOp[r.operation] ??= []).push(r.durationMs); });
Object.entries(byOp).forEach(([op, samples]) => {
  const s = samples.sort((a,b) => a-b);
  const p90 = s[Math.floor(s.length * 0.9)];
  console.log(`${op}: n=${s.length} P90=${p90}ms`);
});
```

### SLO Compliance Interpretation

- **Green (✓)** — P90 is within budget. No action required.
- **Red (✗)** — P90 exceeds budget. Drill into `metadata.messageCount` and `metadata.hwTier` to identify whether the regression is size-driven, hardware-driven, or systemic.
- **`n=0`** — operation has not been exercised in the last 7 days; no data to evaluate.
