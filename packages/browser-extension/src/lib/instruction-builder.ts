...
export function buildInstructionPrompt(params: {
  ...
  const { session, targetPlatform, tier, task, filename, estimatedTokens, caveman, sessionCount = 1, totalMessages } = params
  const msgCount = totalMessages ?? session.messages.length
  const multiLabel = sessionCount > 1 ? ` (${sessionCount} sessions merged)` : ''
  ...
  <session>
    ...
    <messages>${msgCount}</messages>${multiLabel ? `
    <multi_session>${sessionCount}</multi_session>` : ''}
...
Source: ${session.platform} | Messages: ${msgCount} | Tier: ${tierNames[tier]}${multiLabel}
...
**Messages:** ${msgCount} | **Tier:** ${tierNames[tier]}${multiLabel}
