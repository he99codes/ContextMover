...
/**
 * [T6-FIX] Shared sampling strategy — 10% start, 50% middle, 40% recent tail.
 * Used by both offscreen (index jobs) and attention-engine (indexSession) so
 * both paths embed the same representative subset of chunks.
 * Returns the full array unchanged when items.length <= maxItems.
 */
export function sampleForEmbedding<T>(items: T[], maxItems: number): T[] {
  if (items.length <= maxItems) return items;
  const firstCount  = Math.floor(maxItems * 0.10);
  const recentCount = Math.floor(maxItems * 0.40);
  const middleCount = maxItems - firstCount - recentCount;
  const firstIdx  = Array.from({ length: firstCount },  (_, i) => i);
  const recentIdx = Array.from({ length: recentCount },
    (_, i) => items.length - recentCount + i);
  const middleStart = firstCount;
  const middleEnd   = items.length - recentCount;
  const step = (middleEnd - middleStart) / middleCount;
  const middleIdx = Array.from({ length: middleCount },
    (_, i) => Math.floor(middleStart + i * step));
  const allIdx = [...new Set([...firstIdx, ...middleIdx, ...recentIdx])]
    .sort((a, b) => a - b);
  return allIdx.map(i => items[i]);
}
