...
async function runBenchmark(): Promise<{ opsPerMs: number; durationMs: number }> {
  ...
  for (let i = 0; i < RUNS; i++) {
    ...
    results.push(ops / durationMs);
  }

  results.sort((a, b) => a - b);
  const median = results[Math.floor(RUNS / 2)];

  return { opsPerMs: median, durationMs: BENCHMARK_BUDGET_MS };
}
...
async function detectWebGpu(): Promise<boolean> {
  try {
    // Extension pages (sidebar, offscreen) have navigator.gpu but requestAdapter()
    // always returns null and logs "No available adapters." to the console.
    // Skip entirely in extension contexts to avoid console noise.
...
class CapabilityDetector {
  ...
  /** Cached WebGPU availability from initial detection — avoids re-probing. */
  getCachedWebGpu(): boolean {
    return this.cached?.hasWebGpu ?? false;
  }
...
export async function measureSystemHeadroom(): Promise<SystemHeadroom> {
  ...
  // ── GPU — use cached detection result to avoid repeated requestAdapter calls
  // that spam "No available adapters." in contexts where WebGPU is unavailable.
  let gpuAvailable = false;
  try {
    const cached = capabilityDetector.getCachedWebGpu();
    if (cached === true) {
      // Only probe live if the initial detection found a GPU — avoids repeated
      // "No available adapters." console noise in extension sidebar/offscreen.
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
