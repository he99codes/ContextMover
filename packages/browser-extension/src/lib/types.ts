...
export interface ContextSession {
  id: string;                    // Our generated session ID (stable PK)
  nativeId?: string;             // LLM provider's conversation ID (when available)
  platform: Platform;
  title: string;
  customName?: string;
  messages: Message[];
  createdAt: number;
  updatedAt: number;
  metadata?: RequestMetadata;    // Contains conversationId from fetch-interceptor
}
