...
const pendingVaultSyncMap = new Map<string, ContextSession>();
...
export async function queueVaultSync(session: ContextSession, vaultClient: any): Promise<void> {
  pendingVaultSyncMap.set(session.id, session);
  ...
  if (timeSinceLast >= MIN_VAULT_SYNC_INTERVAL) {
    ...
    console.log(`[ContextMover:vault] Sync queued in ${delay}ms for session ${session.id} (${pendingVaultSyncMap.size} pending)`);
  }
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function flushVaultSync(vaultClient: any): Promise<void> {
  if (pendingVaultSyncMap.size === 0) return;
  const sessions = Array.from(pendingVaultSyncMap.values());
  pendingVaultSyncMap.clear();
  ...
  for (const session of sessions) {
    try {
      ...
      console.warn(`[ContextMover:vault] Sync failed for ${session.id}:`, err);
      pendingVaultSyncMap.set(session.id, session);
    }
  }
}
