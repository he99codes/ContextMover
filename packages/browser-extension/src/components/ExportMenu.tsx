...
interface SheetItem {
  ...
  estimate: (session: ContextSession) => number;
}

function estimateSize(bytes: number): string {
...
const ITEMS: SheetItem[] = [
  { id: "markdown",  icon: "\u{1F4C4}", label: "Markdown",        ext: ".md",   action: (s) => exportAsMarkdown(s),       estimate: (s) => new Blob([buildMarkdown(s)]).size },
  { id: "plaintext", icon: "\u{1F4CB}", label: "Plain Text",      ext: ".txt",  action: (s) => exportAsText(s),           estimate: (s) => new Blob([s.messages.map((m) => m.content ?? "").join("\n\n")]).size },
  { id: "json",      icon: "\u{1F5C2}", label: "JSON",            ext: ".json", action: (s) => exportAsJSON(s),           estimate: (s) => new Blob([JSON.stringify(s)]).size },
  { id: "csv",       icon: "\u{1F4CA}", label: "CSV",             ext: ".csv",  action: (s) => exportAsCSV(s),            estimate: (s) => new Blob([s.messages.map((m) => `${m.role},${m.content ?? ""}`).join("\n")]).size },
  { id: "clipboard", icon: "\u{1F4CB}", label: "Copy to Clipboard",             action: (s) => copySessionToClipboard(s), estimate: (s) => new Blob([buildMarkdown(s)]).size },
];
            ...
            {session ? (
              ITEMS.map((item) => (
                ...
                >
                  ...
                  {session && (
                    <span className="text-[9px] text-[#3A3A3A]" style={{ marginRight: 6 }}>
                      {estimateSize(item.estimate(session))}
                    </span>
                  )}
                  {item.ext && (
