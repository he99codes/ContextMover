...
export function installChromeMock(opts: MockChromeOptions = {}): MockChromeHandle {
  ...
  const sendMessage = vi.fn((msg: unknown, callback?: (resp: unknown) => void) => {
    const m = msg as { type?: string };
    let result: unknown;
    if (m?.type === "OFFSCREEN_PING") {
      result = { alive: true, modelReady: true };
    } else {
      result = opts.onSendMessage ? opts.onSendMessage(msg) : undefined;
    }
