...
(() => {
  ...
  // [PERF-H2] Fast hostname pre-filter. The override sits on window.fetch /
  // XMLHttpRequest, so it sees EVERY request the page makes — ads, analytics,
  // fonts, telemetry (e.g. ad.doubleclick.net seen in logs). Those are the vast
  // majority on heavy SPAs like Gemini. A single substring scan against the set
  // of known AI hosts rejects them before any regex runs.
  const _AI_HOST_HINTS = [
    "chatgpt.com", "chat.openai.com", "claude.ai", "anthropic.com",
    "gemini.google.com", "grok.com", "api.x.ai", "deepseek.com", "perplexity.ai",
  ];
  function urlTouchesAIHost(url: string): boolean {
    for (let i = 0; i < _AI_HOST_HINTS.length; i++) {
      if (url.includes(_AI_HOST_HINTS[i])) return true;
    }
    return false;
  }
  ...
  function detectPlatform(url: string): Platform | null {
    if (!url) return null;
    // [PERF-H2] Reject non-AI hosts immediately (ads/analytics/fonts/etc.).
    if (!urlTouchesAIHost(url)) return null;
    // ChatGPT: new endpoint /ces/v1/c/ (completion, 2026+) or legacy /backend-api/conversation/
    // Exclude /ces/v1/t which is a telemetry endpoint — not a conversation API.
    if ((url.includes("chatgpt.com") || url.includes("chat.openai.com")) &&
        (/\/ces\/v1\/c\/|\/backend-api\/conversation\//.test(url))) return "chatgpt";
    ...
    if ((url.includes("grok.com") || url.includes("api.x.ai")) && /\/api\/|\/rest\/|chat|conversation|completion/i.test(url)) return "grok";
  ...
  function extractRequestMetadata(bodyText: string | null, url: string): RequestMetadata | undefined {
    ...
    try {
      ...
      // Also extract from /ces/v1/c/{UUID} — new ChatGPT completion endpoint (2026+)
      if (!meta.conversationId) {
        const cesMatch = url.match(/\/ces\/v1\/c\/([a-zA-Z0-9_-]+)/);
        if (cesMatch) meta.conversationId = cesMatch[1];
      }
  ...
  async function handleAIResponse(platform: Platform, response: Response, requestBody?: string | null): Promise<void> {
    try {
      ...
      if (messages.length === 0) {
        if (text.length > 50) console.debug(`${TAG} ${platform}: no messages parsed (body length=${text.length})`);
        return;
      }
