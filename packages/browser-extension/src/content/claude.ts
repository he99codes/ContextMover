...
// [TITLE-FIX] Strip UI navigation text from Claude titles
function getClaudeTitle(messages: Message[]): string {
  const firstUser = messages.find(m => m.role === 'user')?.content ?? '';
  // Remove UI artifacts: "New chat", "Ctrl+⇧+O", "Chats", "Projects", "Artifacts", etc.
  const cleaned = firstUser
    .replace(/^(New chat|Chats|Projects|Artifacts|CodeUpgrade)[\s:,.]*/i, '')
    .replace(/Ctrl\+⇧\+O/gi, '')
    .replace(/⌘\+⇧\+O/gi, '')
    .trim();
  // [BUG-2 FIX] Cap at 80 chars to prevent sidebar text from creating garbage titles
  const title = cleaned || firstUser.slice(0, 80);
...
getOverridesForPlatform("claude").then((o) => { _localOverrides = o; }).catch(() => {});
...
function scrapeMessages(): Message[] {
  ...
  // Scope queries to the actual chat area. Try multiple scope candidates in
  // priority order; fall back to document when none match.
  const scopeSel = _remoteSelectors?.messageScope;
  let scope: ParentNode;
  if (scopeSel) {
    scope = document.querySelector(scopeSel) ?? document;
  } else {
    scope =
  ...
  // Only runs when no remote config is set. Falls through if 0 results.
  if (!_remoteSelectors?.userSelector && !_remoteSelectors?.assistantSelector &&
      (_localOverrides?.userSelector || _localOverrides?.assistantSelector)) {
    if (_localOverrides.userSelector) {
      scope.querySelectorAll<HTMLElement>(_localOverrides.userSelector)
        .forEach(el => { if (!isStreaming(el)) found.push({ el, role: 'user' }) })
    }
    if (_localOverrides.assistantSelector) {
      scope.querySelectorAll<HTMLElement>(_localOverrides.assistantSelector)
        .forEach(el => { if (!isStreaming(el)) found.push({ el, role: 'assistant' }) })
    }
    if (found.length > 0) {
      console.log(`[CM:claude] local override selectors matched: ${found.length} turns`)
      found.sort((a, b) =>
        a.el.compareDocumentPosition(b.el) & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : 1
      )
      return found
        .map(({ el, role }) => {
          let content = extractContent(el)
          // Self-heal may match wrapper elements (.font-claude-response) — try children
          if (!content.trim()) {
            const child = el.querySelector('[class*="content"], [class*="text"], p, .prose')
            if (child) content = extractContent(child)
          }
          return { role, content, timestamp: Date.now() }
        })
        .filter(m => m.content.trim().length > 0)
    }
    console.debug('[CM:claude] local override selectors returned 0 — falling through to hardcoded')
  }
  ...
  if (found.length === 0) {
    const userSel = '[class*="HumanTurn"], [class*="human-turn"], [class*="UserMessage"], [class*="user-message"]:not([class*="input"])'
    const asstSel = '[class*="AssistantTurn"], [class*="assistant-turn"], [class*="ai-turn"], [class*="AssistantMessage"], [class*="BotMessage"]'
  ...
  if (found.length === 0) {
    console.debug('[CM:claude] all selectors returned 0 — structural fallback will run next')
  }
...
startSessionCapture({
  ...
  getScrollContainerSelector: () => _remoteSelectors?.scrollContainer ?? '[class*="conversation-content"], [class*="flex-col"], main',
  // [ISSUE-5-claude] Enable virtual scroll sweep for long Claude sessions with virtualized history
