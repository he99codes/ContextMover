...
interface Stats { total_users: number; pro_users: number; total_migrations: number; migrations_this_month: number; open_bug_reports: number; }
...
function StatsBar({ token }: { token: string }) {
  ...
  const cards: { label: string; key: keyof Stats }[] = [
    ...
    { label: "Bug Reports Open", key: "open_bug_reports" },
  ];
  return (
    <section>
...
function UsersSection({ token }: { token: string }) {
  ...
  async function action(userId: string, endpoint: string) {
    ...
    setUsers(prev => prev.map(u => {
      ...
      if (endpoint === "grant-pro")   return { ...u, plan: "pro" };
      if (endpoint === "revoke-pro")  return { ...u, plan: "free" };
      if (endpoint === "reset-usage") return { ...u, tier1_used: 0, tier2_used: 0, tier3_used: 0 };
      return u;
    }));
    try {
      ...
      if (j.success) {
        setNotice(`✓ ${endpoint.replace(/-/g, " ")} applied`);
      ...
      <div className="overflow-x-auto rounded border border-[#1A1A1A]">
        <table className="w-full text-xs font-mono">
          ...
          <tbody>
            ...
            {!busy && users.map(u => {
              const rowBusy = pendingRow === u.id;
              return (
              <tr key={u.id} className={`border-b border-[#0F0F0F] hover:bg-[#0A0A0A] transition-opacity ${rowBusy ? "opacity-60" : ""}`}>
                <Td>{u.email}</Td>
                ...
                <td className="px-3 py-2"><div className="flex gap-1 flex-wrap">
                  <SmBtn onClick={() => action(u.id, "reset-usage")} disabled={rowBusy}>{rowBusy ? "…" : "Reset Usage"}</SmBtn>
                  <SmBtn onClick={() => action(u.id, "grant-pro")} disabled={rowBusy}>{rowBusy ? "…" : "Grant Pro"}</SmBtn>
                  <SmBtn danger onClick={() => action(u.id, "revoke-pro")} disabled={rowBusy}>{rowBusy ? "…" : "Revoke Pro"}</SmBtn>
                </div></td>
              </tr>
              );
            })}
