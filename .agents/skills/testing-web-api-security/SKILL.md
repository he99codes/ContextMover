...
- **Rate limiting:** In-memory sliding window in `lib/rate-limiter.ts` (falls back to IP-based key if Supabase unavailable)
...
```
Rate limiter is in-memory and falls back to IP-based keying when Supabase is unavailable.
